...
extension TLPhotoLibrary {
    func getOption(configure: TLPhotosPickerConfigure) -> PHFetchOptions {
        ...
        if configure.allowedImage == false {
            let notImagePredicate = NSPredicate(format: "mediaType != %i", PHAssetMediaType.image.rawValue)
            options.merge(predicate: notImagePredicate)
        }
